  ...
  return (
    <section className="[ media-text__wrapper flow ]">
      {data.allWpCaseStudy.nodes.map( (caseStudy, key) => (
        <>
          <div key={key} className={`[ media-text media-text--large-text ${((key + 1) % 2 === 0) ? 'media-text--reverse' : '' } ] [ grid grid-flow-row sm:grid-flow-col sm:grid-cols-2 md:col-gap-16 ] [ text-white ]`}>
