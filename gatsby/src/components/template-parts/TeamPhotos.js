        ...
        { data.allWpCptTeam.edges.map(( member, key ) => (
          ...
          <div key={ key } className="[ flow ] [ flex flex-col justify-center ] [ text-center text-white ] [ bg-black bg-opacity-25 ]">
            { member.node?.featuredImage ?
              <Img fluid={ member.node.featuredImage.node.remoteFile.childImageSharp.fluid } fadeIn={ true } loading="lazy" alt="" />
              :
              <Img fluid={ data.file.childImageSharp.fluid } fadeIn={ true } loading="lazy" alt="" />
            }
